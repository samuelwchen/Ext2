...
void rmDirEntryUnlink(int dev, MINODE* pmip, MINODE* mip)
{
  ...
  //       break;
  //
  //     freeBlockHelperUnlink(dev, i-11, mip->inode.i_block[i]);
  //     mip->inode.i_block[i] = 0;
  //   }
  // }
  // else
  ...
  for (int i = 0; i < 12; i++)
  {
    ...
    while (dp < (DIR*)(buf + BLKSIZE))
    {
      if (mip->ino == dp->inode)    // target dir to delete found!
      {
        ...
        if (((char *)dp + dp->rec_len >= buf + BLKSIZE) && prevdp != NULL)   // target dir is the last entry in that particular datablock AND there are other records in datablock
        {
          rmEndFileUnlink(dev, dp, prevdp, pmip->inode.i_block[i], buf);
          iput(pmip);
          iput(mip);
          return;
        }
        ...
        {
          rmOnlyFileUnlink(dev, pmip, &(pmip->inode.i_block[i]));
          iput(pmip);
          iput(mip);
          return;
        }
        ...
        {
          rmMiddleFileUnlink(dev, dp, pmip->inode.i_block[i], buf);
          iput(pmip);
          iput(mip);
          return;
        }
...
void freeBlockHelperUnlink(int dev, int level_indirection, int block_num)
{
  ...
  else if (level_indirection > 1)
  {
    ...
    for (int i = 0; i < BLKSIZE/sizeof(int); i++, pIndirect_blk++)
    {
      if (*pIndirect_blk != 0)
      {
        freeBlockHelperUnlink(dev, level_indirection - 1, *pIndirect_blk);
        *pIndirect_blk = 0;
      }
      else
        return;
